// Lavish conversion/summary helpers, adapted under MIT; see THIRD-PARTY-NOTICES.md.
// Shared by the local frame bundle, SQLite boundary and executable unit tests.
...
// requires unique ids; callers suffix only collisions, keeping unambiguous node IDs.
...
export function isTextMetricsExpansion(before, after) {
  if (before.length !== after.length) return false;
  return before.every((element, i) => {
    const next = after[i];
    if (element.type !== 'text' || element.isDeleted || element.autoResize === false) return JSON.stringify(element) === JSON.stringify(next);
    if (next.width < element.width || next.height < element.height) return false;
    return JSON.stringify(element) === JSON.stringify({ ...next, width: element.width, height: element.height });
  });
}
...
function elementLabel(el, boundText) {
  // Excalidraw inserts wrapping newlines even inside words. originalText is
  // the actual edited label, not the canvas layout's broken-word projection.
  const label = el.type === 'text' ? el : boundText.get(el.id) || el;
  const text = String(label.originalText ?? label.text ?? "")
    .replace(/\s+/g, " ")
    .trim();
  return text;
}
